...
class HeikinAshiStrategy(bt.Strategy):        
    params = {
        'fast_ema': 13,                
        'slow_ema': 19,
        'hma_length': 21,                                      # [14, 21, 5, 13, 33, 6, 26, 24, 86, 61, 3, 10, 21, 15, 9, 42, 34, 35, 4, 2, 5, 3, 11, 0, 1, 3, 14, 7, 3]
        'atr_period': 23,                                     # Best parameters (2023-01-17 - 2023-08-04):      
        'atr_threshold': 91,                                  # [13, 19, 21, 23, 91, 2, 19, 4, 15, 86, 0, 18, 11, 9, 10, 21, 13, 14, 16, 13, 20, 11, 15, 5, 6, 1, 7, 20, 8]
        'dmi_length': 2,        
        'dmi_threshold': 19,                                  
        'cmo_period': 4,
        'cmo_threshold': 15,
        'volume_factor_perc': 86,  # Volume must be 2 times the average volume
        'ta_threshold': 0,
        'mfi_period': 18,
        'mfi_level': 11,
        'mfi_smooth': 9,
        ...
        'fastk_period': 11,
        'fastd_period': 15,
        'fastd_matype': 5,      # SMA, EMA, WMA, DEMA, TEMA, TRIMA, KAMA, MAMA, T3
        'mama_fastlimit': 6,
        'mama_slowlimit': 1,
        'apo_fast': 7,
        'apo_slow': 20, 
        'apo_matype': 8,        # SMA, EMA, WMA, DEMA, TEMA, TRIMA, KAMA, MAMA, T3
    ...
    def next(self): 
        ...
        if self.first_run and status == 'LIVE':
        ...
        if self.params.sl_percent != 0:
            ...
            if ((self.in_position > 0) and  (current_price <= (self.long_entry_price * (1 - sl_value)))) or \
                ...
                if self.tg_notifications:
                    asyncio.run(self.notifier.send_message(f"{bt.num2date(self.data1.datetime[0], tz=kiev_tz)}\n"
