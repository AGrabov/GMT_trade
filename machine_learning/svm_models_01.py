...
class SVMModels:
    ...
    def hp_tuning(self, X_train, y_train, X_test, y_test):
        ...
        random_search = RandomizedSearchCV(model, parameters, n_iter=100, cv=5, scoring='r2', verbose=2, n_jobs=-1) 
        ...
        predictions = self.evaluate_svm(best_model, X_test, y_test)
    ...
    def run(self):
        ...
        # model = self.cross_validate_svm(X_train, y_train) if self.settings['use_cross_validation'] else None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', choices=['SVR', 'LinearSVR', 'NuSVR', 'SVC', 'LinearSVC', 'NuSVC', 'OneClassSVM'], 
                        type=str, default='SVC', help='Select model type')
